...
folder_1='C:\Users\labpc2019\Desktop\BehavioralData';
folder_2='C:\Users\labpc2019\Desktop\BehavioralData\data_temp';
folder_save='C:\Users\labpc2019\Desktop\BehavioralData\data';
...
%inpline % test
    function receiveData(~,~,~)
        ...
        if numel(args) >= 2 && ~iscellstr(args(2))
            flag_err=1;
            errmsg='args(2) error'
        end
        if numel(args)>=3
        ...
        if strcmp('VR', Bevt) % data line from VR system (MouseoVeR)
            if (numel(args) < 12)&&(flag_err==0)
                [BC_tstamp,Bevt,vr_time,vr_x,vr_y,vr_z,vr_speed,vr_dir,eventCnt,eventstr_1,eventstr_2,eventstr_3,eventstr_4] = strread(inpline,'%u%s%u%u%u%u%u%s%s%s%s%s%s','delimiter',',');
